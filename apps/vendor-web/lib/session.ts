...
/*
 * Named for this app, not shared with the admin console's `dahab.at`.
 *
 * Cookies are scoped by host, not by port: `localhost:4310` and
 * `localhost:4340` share one jar. With the same names, signing in to one
 * surface would silently sign you out of the other, and this app would pick up
 * an admin's token — a session that belongs to no operator and gets FORBIDDEN
 * on every screen. In production they are different hosts anyway; the
 * distinct names keep development honest too.
 */
export const ACCESS_COOKIE = 'dahab.partner.at';
export const REFRESH_COOKIE = 'dahab.partner.rt';
