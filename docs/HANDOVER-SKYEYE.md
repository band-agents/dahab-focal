...
**Creating an operator** (Operators screen → "Add an operator", added 25 Sep):
`admin.createVendor` in `apps/api/src/routers/admin-onboarding.ts` makes the
centre (`applied`, verification `pending`, no papers), its owner's account
and the owner's `vendorOwner` role scoped to it, in one transaction with an
audit row for each. A duplicate operator name, or an owner email or phone
that already belongs to an account, comes back as `{ ok: false, taken }` and
the form says which. Gated on a new `vendor.create`, plus `role.grant` and
`user.manage` checked inside, because it creates an account and grants a
role. **No password is set here.** The operator page shows the owner and
"cannot sign in yet". Making the owner's login (username and a first
password) is the operator-dashboard branch's `admin-logins.ts`
(`setVendorLogin` works on an existing operator account). Decision taken on
25 Sep: that first password is generated by the server, shown once, and must
be changed at the first sign-in — their branch, as of that day, has the admin
type it instead. The decision still has to reach that branch.

Measured while building it: a query postgres-js has not yet run on a
connection costs **two** round trips (it asks for the parameter types first),
and several such queries on one connection queue rather than pipeline;
`prepare: false` does not change that. Creating an operator takes 1.1–1.7 s
for this reason (the five checks were folded into one statement; the inserts
were not, to keep them readable). With `idle_timeout: 20`, connections — and
their statement cache — are recycled often, so this probably costs every
screen's first load too. Worth its own measured pass.
...
| `neighborhoodKey('eel-garden-quarter')` gave `neighborhood.eelGardenQuarter`, a key that does not exist — two area slugs carry `-quarter` to stay apart from the dive sites of the same name | `neighborhoodKey()` drops the suffix; `apps/admin/tests/nav.test.ts` resolves all six seeded areas |
...
2. **The operator login step** in Sky Eye — a button on the operator page
   calling the operator-dashboard branch's `setVendorLogin`, once that branch
   is merged (see §4, "Creating an operator").
3. **Actions still pointing elsewhere**: refund, ledger reversal, cancelling a
   trip from a booking row (it only exists on the Bookings board).
4. **Translations.** en-GB and ar-EG are complete. **1,112** messages in
   ru/it/fr/es/de are English placeholders declared in
   `$meta.needsTranslation` — visible to the gate, not silently missing.
5. **Never deployed.** Configs exist (`docs/DEPLOY.md`); nothing is online.
6. **PR #1 unmerged.** 57 commits ahead of `master` (25 Sep).
7. **Rotate the seeded passwords** (§2).
8. **Merging with the vendor fork.** The files most likely to conflict are
...
9. `docs/HANDOVER.md` and `docs/SESSION-ADMIN-VENDOR.md` still describe the
...
Last run (25 Sep, after operator creation): 12 tasks green, 1,068 keys × 7 locales.
